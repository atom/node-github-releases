...
class GitHub extends Filters {
  ...
  constructor({user, repo, token}) {
    super()
    this.token = token
    if (user != null) { this.repo = `${user}/${repo}` }
  }

  // Public: List all releases of the repo which matches the {filter}.
  getReleases(filter, callback) {
    if ((callback == null) && filter instanceof Function) { [callback, filter] = Array.from([filter, {}]) }
    ...
    this.callRepoApi(urlPath, (error, releases) => {
      ...
      if (!responseIsArray) { releases = [releases] }
  ...
  downloadAssetOfUrl(url, callback) {
    ...
    inputStream.on('response', response => {
      ...
      if (response.statusCode === 302) {
        this.downloadAssetOfUrl(response.headers.location, callback)
  ...
  callRepoApi(path, callback) {
    ...
    // Set access token.
    if (this.token != null) { options.headers.authorization = `token ${this.token}` }

    request(options, function(error, response, body) {
      var data
      if ((error == null)) {
        data = JSON.parse(body)
        if (response.statusCode !== 200) { error = new Error(data.message) }
      }
      callback(error, data)
    })
  }
